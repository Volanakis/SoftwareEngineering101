...
- [x] Τελικό integration test pass και οι δύο μαζί — `tests/test_screening_workflow.py` (happy path CREATED→ANNOUNCED) + `tests/test_integration_lifecycle.py` (ένα πρόγραμμα, 3 προβολές → SCHEDULED / manual REJECTED / auto REJECTED, HTTP-level, και τα δύο blueprints). Πλήρης σουίτα: **183 tests, όλα πράσινα**.
